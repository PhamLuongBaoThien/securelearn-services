...
export const validateTemplate = (event: string, type: string, subject: string | undefined, body: string) => {
  if (!TEMPLATE_EVENTS.includes(event as any)) throw new Error('Sự kiện template không hợp lệ.');
  if (type === 'EMAIL' && !subject?.trim()) throw new Error('Email template phải có tiêu đề.');
  const found = [...extractVariables(subject || ''), ...extractVariables(body)];
  const invalid = found.filter(variable => !(VARIABLE_WHITELIST[event] || []).includes(variable));
  if (invalid.length) throw new Error(`Biến template không hợp lệ: ${[...new Set(invalid)].join(', ')}`);
...
class TemplateService {
  ...
  async create(input: any) { return NotificationTemplate.create({ ...input, variables: validateTemplate(input.event, input.type, input.subject, input.body) }); }
  async update(id: string, input: any) { const current = await NotificationTemplate.findById(id); if (!current) throw new Error('Template không tồn tại.'); const next = { event: input.event ?? current.event, type: input.type ?? current.type, subject: input.subject ?? current.subject, body: input.body ?? current.body }; Object.assign(current, input, { variables: validateTemplate(String(next.event), String(next.type), next.subject as string | undefined, String(next.body)) }); return current.save(); }
  async remove(id: string) { const result = await NotificationTemplate.findByIdAndDelete(id); if (!result) throw new Error('Template không tồn tại.'); }
