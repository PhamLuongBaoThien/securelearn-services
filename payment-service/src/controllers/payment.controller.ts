...
class PaymentController {
  public async courseCheckout(req: AuthRequest, res: Response): Promise<void> {
    try {
      const { paymentMethod, provider, couponCode, checkoutMode, courseId } = req.body as { paymentMethod?: PaymentMethod; provider?: PaymentProvider; couponCode?: string; checkoutMode?: 'CART' | 'BUY_NOW'; courseId?: string };
      if (!paymentMethod) {
      ...
      const result = await paymentService.createCourseCheckout(
        ...
        authHeader,
        { paymentMethod, provider, couponCode, checkoutMode, courseId },
